...
"""
        ...
        o = OpenWrt(native=native, dsa=False)
...
"""
    ...
    def test_render_multiple_ip(self):
        o = OpenWrt(self._multi_ip_netjson, dsa=False)
        expected = self._tabs(self._multi_ip_uci)
        self.assertEqual(o.render(), expected)

    def test_parse_multiple_ip(self):
        o = OpenWrt(native=self._multi_ip_uci, dsa=False)
...
"""
    ...
    def test_render_simple_bridge(self):
        o = OpenWrt(self._simple_bridge_netjson, dsa=False)
...
"""
    ...
    def test_render_complex_bridge(self):
        o = OpenWrt(self._complex_bridge_netjson, dsa=False)
        self.assertEqual(o.render(), self._tabs(self._complex_bridge_uci))

    def test_parse_complex_bridge(self):
        o = OpenWrt(native=self._tabs(self._complex_bridge_uci), dsa=False)
...
"""
    ...
    def test_empty_interface(self):
        o = OpenWrt({"interfaces": [{"name": "eth0", "type": "ethernet"}]}, dsa=False)
        expected = self._tabs(
            """package network
...
"""
    ...
    def test_ifname_length(self):
        o = OpenWrt(
            {"interfaces": [{"name": "ifname0123456789", "type": "ethernet"}]},
            dsa=False,
        )
        with self.assertRaises(ValidationError):
            o.validate()
    ...
    def test_ifname_pattern(self):
        o = OpenWrt({"interfaces": [{"name": "eth 0", "type": "ethernet"}]}, dsa=False)
        with self.assertRaises(ValidationError):
            o.validate()
        # ensure fix works
...
"""
    ...
    def test_network_dot_conversion(self):
        o = OpenWrt(
            {
                "interfaces": [
                    {"name": "eth0.1", "type": "ethernet", "network": "lan.1"}
                ]
            },
            dsa=False,
        )
